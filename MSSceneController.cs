...
public class MSSceneController : MonoBehaviour {
    ...
    IEnumerator limit()
    {
        if (vehicleCode.KMh * clampGear > 70 && chk)
        {
            Global.Score -= 5;
            chk = false;
        }
        else if (vehicleCode.KMh * clampGear < 70)
        {
            chk = true;
    ...
    void Update () {
		if (!error) {
			...
			velocitykmh = "km/h: " + (int)(vehicleCode.KMh * clampGear);
		...
		if (!error) {
			...
			if (vehicles.Length > 0 && currentVehicle < vehicles.Length && GUIVisualizer && vehicleCode) {
				if (vehicleCode.isInsideTheCar) {
                    ...
                    GUI.Label(new Rect(10, 50, 300, 35), score);
                    GUI.Label(new Rect(10, 90, 300, 35), velocitykmh);
                }
			}
		}
	}
