...
public class MSVehicleController : MonoBehaviour {
...

	{
...

		{
			...
			if (LBlinker == false && RBlinker == false)
...

	{
		...
		if (Input.GetKeyDown(KeyCode.Joystick1Button5) || Input.GetKeyDown(KeyCode.RightShift))

		{

			if (RBlinker == false && LBlinker == false)
	...
	void Update(){
		...
		StartCoroutine("LeftBlinker");

		StartCoroutine("RightBlinker");
    ...
    {
        if (other.transform.tag == "NOROADS")
        {
            SceneManager.LoadScene("end_noroad");
        }

        if (other.transform.tag == "End")
        {
            SceneManager.LoadScene("end_suc");
        }
    }

    void OnTriggerExit(Collider other)
    {
        if (other.transform.tag == "inspectW" && other.transform.parent.parent.GetComponent<Traffic>().WR1.gameObject.activeSelf)
        {
            Global.Score -= 5;
        }
        if (other.transform.tag == "inspectE" && other.transform.parent.parent.GetComponent<Traffic>().ER1.gameObject.activeSelf)
        {
            Global.Score -= 5;
        }
        if (other.transform.tag == "inspectN" && other.transform.parent.parent.GetComponent<Traffic>().NR1.gameObject.activeSelf)
        {
            Global.Score -= 5;
        }
        if (other.transform.tag == "inspectS" && other.transform.parent.parent.GetComponent<Traffic>().SR1.gameObject.activeSelf)
        {
            Global.Score -= 5;
        }


		if (other.transform.tag == "Uturn" && (other.transform.parent.parent.GetComponent<Traffic>().SA1.gameObject.activeSelf == false || LBlinker == false))
		{
			Global.Score -= 5;
		}
    }
